...
def final():
    ...
    def recommend(name, dur):
        code = 0
        data = data_frame[data_frame['type'] == "TV Show"]
    ...
    tv_dur_options = [3, 4, 5, 'Any']
    st.subheader("Recommendations for TV Shows")
    temp_dur_tv = st.select_slider("Choose duration limit (Number of Seasons)", options=tv_dur_options)
    user_input = st.selectbox('Enter the title', data_frame['title'].values)

    if st.button('Recommend'):
        output_title, output_ac1, output_ac2, output_ac3, output_dir, output_dur, output_descrip, posters = recommend(
            user_input, temp_dur_tv)
